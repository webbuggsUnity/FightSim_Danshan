...
namespace EmeraldAI
{
    public class EmeraldAIEventsManager : MonoBehaviour
    {
        ...
        /// 
        GameObject temp;
        public void RightStepDust()
        {
            if (temp != null)
            {
                Destroy(temp);
            }
            temp = Instantiate(GetItem(0), GetItem(1).transform.position, Quaternion.identity);
            temp.transform.parent = null;
        } 
        
        public void LeftStepDust()
        {
            if (temp != null)
            {
                Destroy(temp);
            }
            temp = Instantiate(GetItem(0), GetItem(2).transform.position, Quaternion.identity);
            temp.transform.parent = null;
        }
        ...
        public GameObject GetItem(int ItemID)
        {
            ...
            for (int i = 0; i < EmeraldComponent.ItemList.Count; i++)
            {
                if (EmeraldComponent.ItemList[i].ItemID == ItemID)
                {
                    if (EmeraldComponent.ItemList[i].ItemObject != null)
                    {
                        GameObject temp = EmeraldComponent.ItemList[i].ItemObject;
                        return temp;
                    }
                }
            }
